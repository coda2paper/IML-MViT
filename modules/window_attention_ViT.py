...
class SimpleFeaturePyramid(nn.Module):
    ...
    def __init__(self,
        ...
        self.residual9 = ResBottleneckBlock(
            in_channels=out_channels,
            out_channels=out_channels,
            bottleneck_channels=out_channels // 8,
            norm="LN",
            act_layer=nn.GELU,
        )
        self.residual12 = ResBottleneckBlock(
            in_channels=out_channels,
            out_channels=out_channels,
            bottleneck_channels=out_channels // 16,
            norm="LN",
            act_layer=nn.GELU,
        )
    ...
    def forward(self, x):
        ...
        results = []
        x4 = self.stages[0](f12)
        results.append(x4)

        x4 = self.stages[1](f12)
        results.append(x4)

        x1 = self.stages[2](f3)
        x2 = self.stages[2](f6)
        x3 = self.stages[2](f9)
        x4 = self.stages[2](f12)
        x4 = self.linear_fuse(torch.cat([x1, x2, x3, x4], dim=1))
        x4 = self.residual9(x4)
        results.append(x4)

        x1 = self.stages[3](f3)
        x2 = self.stages[3](f6)
        x3 = self.stages[3](f9)
        x4 = self.stages[3](f12)
        x4 = self.linear_fuse(torch.cat([x1, x2, x3, x4], dim=1))
        x4 = self.residual12(x4)
        results.append(x4)
